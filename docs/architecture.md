...
| Knowledge Graph (bonus) | `src/knowledge_graph/` | `build_graph.py` constructs an in-memory NetworkX graph (Judgment/Provision/Instrument/LegalDomain/ThematicArea/MemberState/Chamber/LegalPrinciple nodes; cites/interprets/belongs_to/concerns/in_domain/referred_by/decided_by/invokes edges) via rule-based extraction over the Step 1 JSON. `graph_tools.py` exposes `expand_via_graph` as a 4th tool both agent systems pick up automatically when the graph is built, deliberately not domain-restricted so it can surface the environmental↔agricultural bridge a domain-scoped vector search would miss. See `src/knowledge_graph/README.md` for the full schema and construction pipeline writeup the guide asks for. |
